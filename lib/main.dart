...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    DropTarget droper = DropTarget(
      ...
      child: const Image( height: 28, width: 50, image: AssetImage('graphics/upload.jpg')),
    );
    ...
    for (Page item in _list) {
      ...
      list.add(Column(children: [
        InkWell(
            onTap: () {_handlePreview(image);},
            child: Ink.image(image: image, height: 200, width: 150)),
        SizedBox(
            width: 150,
            child: Row(
              children: [
                Checkbox( value: _list[x].checked, onChanged: (value) { setState(() { _list[x].checked = value!; }); },),
    ...
    if (preview == null) {
      child = Column(
        children: [
          Row(children: [
            droper,
            ElevatedButton( onPressed: _handleCleanList, child: const Text("删除所有")),
            ElevatedButton( onPressed: save, child: const Text("导出")),
          ]),
          Expanded(child: SizedBox(width: double.infinity, child: SingleChildScrollView(child: Wrap( spacing: 8.0, runSpacing: 8.0, children: list,)))),
        ],
      );
    } else {
      child = InkWell( onTap: _handleClosePreview, child: Ink.image(image: preview!));
    }
